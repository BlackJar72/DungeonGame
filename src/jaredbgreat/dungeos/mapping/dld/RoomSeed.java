...
public class RoomSeed {
	...
	public Room growRoomX(int xdim, int zdim, int height, 
		...
		if(dungeon.room[x][z] != container) return null;
			while((canAddX || canSubX) && grownX < xdim) {
				if((endX+1) >= dungeon.size.width) canAddX = false;
				if((beginX-1) < 0) canSubX = false;
				if(canAddX) {
					if(dungeon.room[endX+w][z] == container) {
						grownX += 1;
						endX += 1;
					} else canAddX = false;
				} if(canSubX) {
					if(dungeon.room[beginX-w][z] == container) {
						grownX += 1;
						beginX -= 1;
					} else canSubX = false;
				}
			...
			while((canAddZ || canSubZ) && grownZ < zdim) {
				...
				for(int i = lowEdge; i <= highEdge; i++) {
					if((endZ+1) >= dungeon.size.width) canAddZ = false;
						...
						if(dungeon.room[i][endZ+w] != container) canAddZ = false;
					}
					if((beginZ-1) < 0) canSubZ = false;
					else {
						if(dungeon.room[i][beginZ-w] != container) canSubZ = false;								
					}
				}
				if(canAddZ) {
					grownZ += 1;
		...
		if((grownX < 1) || (grownZ < 1)) {
			return null; // Not big enough for a room!
		}
		else {
			return new Room(dungeon.rooms.size(), beginX, endX, beginZ, endZ, 
                                dungeon.localModel[dungeon.random.nextInt(dungeon.localModel.length)],
                                dungeon)
                                    .build(dungeon.room, dungeon.geomodel);
		}
	}
	...
	public Room growRoomZ(int xdim, int zdim, int height, 
		...
		if(dungeon.room[x][z] != container) return null;
			while((canAddZ || canSubZ) && grownZ < zdim) {
				...
				if((beginZ-1) < 0) canSubZ = false;
				if(canAddZ) {
					if(dungeon.room[x][endZ+w] == container) {
						grownZ += 1;
						endZ += 1;
					} else canAddZ = false;
				} if(canSubZ) {
					if(dungeon.room[x][beginZ-w] == container) {
						grownZ += 1;
						beginZ -= 1;
					} else canSubZ = false;
				}
			...
			while((canAddX || canSubX) && grownX < xdim) {
				...
				for(int i = lowEdge; i <= highEdge; i++) {
					if((endX+1) >= dungeon.size.width) canAddX = false;
						...
						if(dungeon.room[endX+w][i] != container) canAddX = false;
					}
					if((beginX-1) < 0) canSubX = false;
					else {
						if(dungeon.room[beginX-w][i] != container) canSubX = false;								
					}
				}
				if(canAddX) {
					grownX += 1;
